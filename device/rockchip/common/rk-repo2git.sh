...
function merge_into_temi()
{
	git checkout -b temi-master
	git merge kernel    --squash; git commit -m "Merge [kernel](ad5050d) into temi-master"
	git merge u-boot    --squash; git commit -m "Merge [u-boot](21be399) into temi-master"
	git merge rkbin     --squash; git commit -m "Merge [rkbin](70234e2) into temi-master"
	git merge prebuilts --squash; git commit -m "Merge [prebuilts](b93c745) into temi-master"
	git merge tools     --squash; git commit -m "Merge [tools](98b9d0d) into temi-master"
	git merge device    --squash; git commit -m "Merge [device](5aa7d0d + 1) into temi-master"
	git merge docs      --squash; git commit -m "Merge [docs](f143655) into temi-master"
	git merge ubuntu    --squash; git commit -m "Merge [ubuntu](1becd76) into temi-master"
}

function new_temi_branch()
{
	git checkout -b TEMI_kernel
	git checkout -b TEMI_u-boot
	git checkout -b TEMI_rkbin
	git checkout -b TEMI_prebuilts
	git checkout -b TEMI_tools
	git checkout -b TEMI_device
	git checkout -b TEMI_docs
	git checkout -b TEMI_ubuntu
	git checkout -b temi-master
}

function main()
{
	while getopts ":itmn" opt > /dev/null; do
        case "$opt" in
            i) init_git ;;
            t) repo_to_git ;;
            m) merge_into_temi ;;
            n) new_temi_branch ;;
            ?) echo "Usage: ./$0 [-i] [-t []] [-m] [-n]"
               exit 1
               ;;
        esac
    done
